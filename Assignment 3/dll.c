...
void insert_after(node *target,node *pre)
{
    node *cur=pre->next;
    pre->next=target;
    target->prev=pre;
...
void delete_after(node *pre)
{
    if(pre->next==NULL) printf("No node available to be deleted.\n");
    else{
        node *cur=(pre->next),*temp=cur->next;
        temp->prev=pre;
        pre->next=temp;
        free(cur);
    }
...
{
	if(head1==NULL || head2==NULL) printf("One of the list is empty. Operation can't be done"\n);
	else
	{
		node *temp=head1;
		while(temp->next!=NULL) temp=temp->next;
